...
const updateProductDetails = asyncHandler(async (req, res) => {
  try {
    const { name, description, price, category, countInStock, brand } =
      req.body;
    const image = req.file?.path; // If you use multer to handle file uploads
    ...
    if (!countInStock) return res.json({ error: "Quantity is required" });

    const product = await Product.findByIdAndUpdate(
      req.params.id,
      { name, description, price, category, countInStock, brand, image },
      { new: true }
    );
