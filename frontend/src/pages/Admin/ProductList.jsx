...
const ProductList = () => {
  ...
  const uploadFileHandler = async (e) => {
    ...
    for (let i = 0; i < e.target.files.length && i < 4; i++) {
      formData.append("additionalImages", e.target.files[i + 1]);
    }

    try {
      const res = await uploadProductImage(formData).unwrap();
      toast.success(res.message);
      setImage(res.imageUrls.main);
      setImageUrl(res.imageUrls.main);
      setAdditionalImages(res.imageUrls.additional);
    } catch (error) {
      toast.error(error?.data?.message || error.error);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      additionalImages.forEach((img, index) => {
        productData.append(`additionalImage${index + 1}`, img);
      });
      productData.append("name", name);
      productData.append("description", description);
      productData.append("price", price);
              ...
              <div className="flex items-center justify-center w-full">
                <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 transition-colors duration-300">
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p className="text-xs text-gray-500 mt-2">
                      PNG, JPG, GIF up to 10MB (1 main + 4 additional)
                    </p>
                  </div>
                  <input
                    id="image"
